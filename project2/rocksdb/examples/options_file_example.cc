...
int main() {
  ...
  db_opt.statistics = rocksdb::CreateDBStatistics();
  ...
  std::cout << "InstrMuLocks b/ cleanup: " << db_opt.statistics->getTickerCount(INSTRUMENTED_MUTEX_LOCK) << std::endl;
  ...
  delete db;
  std::cout << "InstrMuLocks a/ cleanup: " << db_opt.statistics->getTickerCount(INSTRUMENTED_MUTEX_LOCK) << std::endl;
